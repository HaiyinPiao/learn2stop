...
from models.mlp_disc_lts import DiscLtsPolicy
...
if args.model_path is None:
    if is_disc_action:
        ...
        policy_net = DiscLtsPolicy(state_dim, env.action_space.n)
    else:
        print("Only discrete action supported!")
        exit(0)
...
def update_params(batch, i_iter):
    ...
    for _ in range(optim_epochs):
        ...
        states, actions, returns, advantages, stops, fixed_log_probs, fixed_lts_log_probs = \
            states[perm].clone(), actions[perm].clone(), returns[perm].clone(), advantages[perm].clone(), stops[perm].clone(), fixed_log_probs[perm].clone(), fixed_lts_log_probs[perm].clone()

        for i in range(optim_iter_num):
            ind = slice(i * optim_batch_size, min((i + 1) * optim_batch_size, states.shape[0]))
            states_b, actions_b, advantages_b, returns_b, stops_b, fixed_log_probs_b, fixed_lts_log_probs_b = \
                states[ind], actions[ind], advantages[ind], returns[ind], stops[ind], fixed_log_probs[ind], fixed_lts_log_probs[ind]

            ppo_step(policy_net, value_net, optimizer_policy, optimizer_value, 1, states_b, actions_b, returns_b, 
                     advantages_b, stops_b, fixed_log_probs_b, fixed_lts_log_probs_b, args.clip_epsilon, args.l2_reg)
